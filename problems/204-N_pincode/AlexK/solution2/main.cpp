...
struct SegmentTree {
  ...
  int64_t Left(int64_t node) const { return node * 2 + 1; }

  int64_t Right(int64_t node) const { return node * 2 + 2; }
  ...
  int64_t Query(int64_t l, int64_t r, int64_t node, int64_t node_l,
                int64_t node_r) {
    if (l <= node_l && node_r <= r) {
      return tree[node].value + tree[node].promise;
    }
  ...
  int64_t Query(int64_t l, int64_t r) { return Query(l, r, 0, 0, n - 1); }
};
...
int main() {
  ...
  for (int64_t i = 0; i < requests; ++i) {
    ...
    --x2;
    --y2;
    std::cout << table.Query2D(y1, y2, x1, x2) << '\n';
  }
}
