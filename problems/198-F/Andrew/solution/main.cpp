...
struct numeric_limits<pair<F, S>> {
  static constexpr pair<F, S> denorm_min() noexcept {  // NOLINT
    return {numeric_limits<F>::denorm_min(), numeric_limits<S>::denorm_min()};
  }
};
template <class F, class S>
auto sum(const std::pair<F, S>& lhs, const std::pair<F, S>& rhs) {  // NOLINT
...
class SegmentTree {
  ...
  struct PromiseType {
   public:
    ...
    auto operator*=(const PromiseType& other) {
      promise_ = std::sum(promise_, other.promise_);
      return *this;
    }
 ...
 public:
  ...
  void Push(PointerType node) {
    ...
    // std::cout << fmt % node % At(node).value << " end" << std::endl;
  }
