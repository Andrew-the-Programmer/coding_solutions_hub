...
std::string Point::ToString() const {
  ...
  return "Point("s + std::to_string(x) + ", " + std::to_string(y) + ")"s;
}
...
bool Ray::CrossesSegment(const Segment& s) const {
  bool line_crosses = Line(*this).CrossesSegment(s);
  if (!line_crosses) {
    return false;
  }
  auto a = V(start_point, s.first_point);
  auto b = V(start_point, s.second_point);
  auto c = directing_vector;
  auto vp_ac = std::abs(V::SignedArea(a, c));
  auto vp_bc = std::abs(V::SignedArea(b, c));
  auto sp_bc = V::ScalarProduct(b, c);
  auto sp_ac = V::ScalarProduct(a, c);
  return vp_ac * sp_bc + vp_bc * sp_ac > 0;
}
...
bool Polygon::ContainsPoint(const Point& p) const {
  auto ray = Ray(p, Vector(std::numeric_limits<size_t>::max(), 1));
  size_t count = 0;
  for (const auto& segment : Segments()) {
    if (segment.first_point == p) {
      return true;
    }
...
std::string Polygon::ToString() const {
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    repr += (i == 0 ? "" : ", ") + (boost::format("%1%") % points[i]).str();
  }
  repr += ")";
  return repr;
}
