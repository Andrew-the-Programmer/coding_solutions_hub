...
auto Min(const A& a, const B& b, const Tail&... tail) {
  return Min((a < b ? a : b), tail...);
}
...
struct Ray {
  ...
  Ray(const P& start_point, const P& end_point) : Ray(start_point, V(start_point, end_point)) {
  }
  ...
  bool PointsTo(const P& point) {
    return V::ScalarProduct(V(start_point, point), directing_vector) >= 0;
  }
...
struct Section {
  ...
  auto GetRayFromFirstPointToSecondPoint() const {
    return Ray<T>(first_point, second_point);
  }

  auto GetRayFromSecondPointToFirstPoint() const {
    return Ray<T>(second_point, first_point);
  }
...
template <class T>
auto Distance(const Point<T>& point, const Section<T>& section) {
  auto section_vector = section.GetDirectingVector();
  auto ray1 = Ray<T>(section.second_point, section_vector);
  if (ray1.PointsTo(point)) {
    return Vector<T>(section.second_point, point).Length();
  }
  auto ray2 = Ray<T>(section.first_point, -section_vector);
  if (ray2.PointsTo(point)) {
    return Vector<T>(section.first_point, point).Length();
  }
  auto vec_first_point = Vector<T>(section.first_point, point);
  auto sin_angle = std::abs(Vector<T>::Sin(section_vector, vec_first_point));
  return sin_angle * vec_first_point.Length();
}

template <class T>
auto Distance(const Section<T>& first, const Section<T>& second) {
  if (Section<T>::Cross(first, second)) {
    return .0;
  }
  return Min(Distance(first.first_point, second), Distance(first.second_point, second),
             Distance(second.first_point, first), Distance(second.second_point, first));
}

template <class T>
void Solution(const Section<T>& first, const Section<T>& second) {
  Output(Distance(first, second));
}
